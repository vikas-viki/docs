...
unlike `var`, the variables `let, const & arrow functions` are not hoisted (they are hoisted, but not in global scope), meaning, they are not allocated with the memory, until, the line in which they are declared are executed.
